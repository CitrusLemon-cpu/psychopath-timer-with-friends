...
function renderTimers() {
  ...
  timerList.innerHTML = state.timers.map((timer) => {
    ...
    return `<article class="timer-card ${complete ? 'complete' : timer.paused ? 'paused' : ''}" style="--timer-color:${timer.color || '#54d6d2'}"><div class="timer-top"><div><div class="timer-title">${escape(timer.name)}</div><div class="timer-crew">${escape(crew)}</div></div><span class="status">${status}</span></div><div class="progress-row"><div class="progress-track"><div class="progress-fill" style="width:${percent}%"></div></div><span class="time-left">${duration(remaining)}</span></div><div class="timer-bottom"><span>${formatDate(timer.start)} → ${formatDate(timer.end)}</span><div class="timer-actions">${!complete && !standby ? `<button data-toggle="${timer.id}">${timer.paused ? 'RESUME' : 'PAUSE'}</button>` : ''}<button data-edit="${timer.id}">EDIT</button><button data-delete="${timer.id}">DELETE</button></div></div></article>`;
  }).join('');
  emptyState.classList.toggle('hidden', state.timers.length > 0); $('#active-count').textContent = String(active).padStart(2, '0');
}
function updateColorLabel() { $('#color-value').textContent = $('#timer-color').value.toUpperCase(); }
...
const localInputValue = (iso) => { const date = new Date(iso); const pad = (value) => String(value).padStart(2, '0'); return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`; };
function openDialog(timer = null) { editingId = timer?.id || null; renderPeople(); $('#timer-form').reset(); if (timer) { $('#timer-name').value = timer.name; $('#timer-start').value = localInputValue(timer.start); $('#timer-end').value = localInputValue(timer.end); $('#timer-color').value = timer.color || '#54d6d2'; updateColorLabel(); (timer.people || []).forEach((id) => { const checkbox = document.querySelector(`input[name="crew"][value="${id}"]`); if (checkbox) checkbox.checked = true; }); } else { const start = new Date(); const end = new Date(start.getTime() + 60 * 60 * 1000); $('#timer-start').value = localInputValue(start); $('#timer-end').value = localInputValue(end); updateColorLabel(); } dialog.showModal(); }
...
$('#timer-form').addEventListener('submit', (event) => { event.preventDefault(); const end = new Date($('#timer-end').value); const startValue = $('#timer-start').value; const start = startValue ? new Date(startValue) : new Date(); if (end <= start) return alert('End time must be after the start time.'); const name = $('#timer-name').value.trim() || end.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }); const values = { name, start: start.toISOString(), end: end.toISOString(), color: $('#timer-color').value, people: [...document.querySelectorAll('input[name="crew"]:checked')].map((input) => input.value) }; if (editingId) Object.assign(state.timers.find((timer) => timer.id === editingId), values); else state.timers.push({ id: crypto.randomUUID(), ...values, paused: false }); editingId = null; save(); render(); dialog.close(); });
timerList.addEventListener('click', (event) => { const toggle = event.target.closest('[data-toggle]'); const edit = event.target.closest('[data-edit]'); const remove = event.target.closest('[data-delete]'); if (toggle) { const timer = state.timers.find((item) => item.id === toggle.dataset.toggle); if (timer.paused) { const pausedDuration = Date.now() - timer.pausedAt; timer.start = new Date(new Date(timer.start).getTime() + pausedDuration).toISOString(); timer.end = new Date(new Date(timer.end).getTime() + pausedDuration).toISOString(); delete timer.pausedAt; timer.paused = false; } else { timer.pausedAt = Date.now(); timer.paused = true; } save(); render(); } if (edit) openDialog(state.timers.find((item) => item.id === edit.dataset.edit)); if (remove) { state.timers = state.timers.filter((item) => item.id !== remove.dataset.delete); save(); render(); } });
$('#timer-color').addEventListener('input', updateColorLabel); ['#timer-start', '#timer-end'].forEach((selector) => $(selector).addEventListener('click', (event) => { if (typeof event.currentTarget.showPicker === 'function') event.currentTarget.showPicker(); }));
