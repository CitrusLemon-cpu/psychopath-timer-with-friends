...
- The client leaves every joined room before an explicit anonymous sign-out so capacity is released. Abruptly abandoned guest sessions still need a future server-side TTL cleanup job; offline anonymous profiles are hidden from the live manifest in the meantime.
- Invite codes are identity-neutral revocable grants, not role grants: a permanent user derives `member`, while an anonymous user derives `guest`; moderator promotion is a later owner action. Codes are unique, bounded by use count, optionally user-bound, and may expire. Invite-only rooms redeem them atomically; approval-required rooms convert them into pending requests and consume a use only during locked approval.
...
- Permanent accounts lazily provision exactly one private personal room, pinned separately by the client and restricted to personal timers and its owner. Anonymous guests get the same workspace from browser storage without creating server-side room data.
