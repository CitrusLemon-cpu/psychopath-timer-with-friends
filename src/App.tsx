...
export default function App() {
      ...
      setState((current) => {
        ...
        const rooms = current.rooms.map((room) => {
          ...
          return {
            ...
            activity: [...completed.map((timer) => ({ id: crypto.randomUUID(), timer, completedAt: timer.endAt })), ...room.activity].slice(0, 25),
          }
        })
  ...
  return (
    <div className="shell">
      ...
      {activeRoom ? <RoomView room={activeRoom} currentUserId={state.currentUserId} now={now} onBack={() => setState((current) => ({ ...current, activeRoomId: null }))} onSaveTimer={saveTimer} onToggleTimer={(id) => updateActiveRoom((room) => ({ ...room, timers: room.timers.map((timer) => timer.id === id ? toggleTimer(timer, now) : timer) }))} onDeleteTimer={(id) => updateActiveRoom((room) => ({ ...room, timers: room.timers.filter((timer) => timer.id !== id) }))} onRunAgain={(id) => updateActiveRoom((room) => { const source = room.activity.find((item) => item.id === id); if (!source) return room; const duration = source.timer.endAt - source.timer.startAt; return { ...room, timers: [{ ...source.timer, id: crypto.randomUUID(), startAt: now, endAt: now + duration, pausedAt: null }, ...room.timers] } })} onSendMessage={(message: ChatMessage) => updateActiveRoom((room) => ({ ...room, chat: [...room.chat, message].slice(-50) }))} /> : <Dashboard rooms={state.rooms} now={now} onOpenRoom={(id) => setState((current) => ({ ...current, activeRoomId: id, rooms: current.rooms.map((room) => room.id === id ? { ...room, lastVisitedAt: now } : room) }))} onCreateRoom={createRoom} onJoinRoom={(code) => { const room = state.rooms.find((item) => item.code === code); if (!room) return false; setState((current) => ({ ...current, activeRoomId: room.id })); return true }} />}
