  ...
  return (
    <main className="room-view">
      <section className="room-header panel">
        ...
        <div className="room-identity"><p className="eyebrow">// ACTIVE CONTROL ROOM</p><h1>{room.name}</h1><div><span className="signal"><i /> {connectionLabel}</span><span>ROOM {room.code}</span><span>{room.deck}</span></div></div>
      ...
      <div className="workspace">
        ...
        <aside className={`right-rail ${mobileRail ? 'mobile-open' : ''}`}>
          ...
          <section className={`rail-panel panel chat-panel ${mobileRail === 'crew' ? 'mobile-hidden' : ''}`}><div className="aside-heading"><div><p className="eyebrow">// {isRemote ? 'PRIVATE ROOM CHANNEL' : 'LOCAL CHANNEL'}</p><h2>CREW COMMS</h2></div><span className="transmit">TX</span></div><div className="messages">{room.chat.slice(-8).map((item) => { const sender = room.crew.find((member) => member.id === item.senderId); return <div className={`message ${item.senderId === currentUserId ? 'mine' : ''}`} key={item.id}><div><strong>{sender?.name ?? 'Unknown'}</strong><span>{new Date(item.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div><p>{item.text}</p></div>})}</div><form className="chat-form" onSubmit={sendMessage}><input value={message} onChange={(event) => setMessage(event.target.value)} maxLength={240} placeholder="Transmit message..." aria-label="Chat message" /><button aria-label="Send message">→</button></form><p className="local-note">{isRemote ? 'PLAIN ROOM CHAT · REPLIES AND REACTIONS DEFERRED' : 'LOCAL DEVICE CHANNEL · NOT REALTIME'}</p></section>
        </aside>
      </div>
